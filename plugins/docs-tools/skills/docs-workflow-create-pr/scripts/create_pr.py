...
def commit_and_push(
    branch: str, identifier: str, base_path: str, remote: str = "upstream",
) -> bool:
    """Stage, commit, and push documentation changes.

    Stages documentation files and workflow tracking files (progress,
    markers) but excludes process artifacts (analysis, requirements,
    plan, reviews) from the commit.
    """
    ...
    if status.stdout.strip():
        ...
        for line in status.stdout.strip().splitlines():
            ...
            if path.startswith(bp):
                # Inside base_path — only include workflow/ (progress files)
                if path.startswith(workflow_prefix):
                    files_to_stage.append(path)
            else:
                # Outside base_path — doc files, markers, nav updates, etc.
                files_to_stage.append(path)

        if files_to_stage:
            subprocess.run(["git", "add", "--"] + files_to_stage, check=True)
            message = f"docs: {identifier}\n\nGenerated by docs-workflow pipeline."
    ...
    raise RuntimeError(
        ...
        f"Ensure push access is configured for '{remote}' or 'origin'."
    )
...
def main():
    ...
    commit_and_push(branch, args.identifier, args.base_path, args.remote)
