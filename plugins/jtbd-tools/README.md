...
A Claude Code plugin for Jobs-To-Be-Done (JTBD) documentation analysis. Extract user goals from technical documentation, generate JTBD-oriented TOCs, compare current vs proposed structures, and produce stakeholder-facing consolidation reports.

## Skills

All skills are invoked as `jtbd-tools:<skill-name>`:

| Skill | Description |
|-------|-------------|
| `jtbd-tools:jtbd-analyze` | Extract JTBD records from markdown documentation |
| `jtbd-tools:jtbd-analyze-adoc` | Extract JTBD records from AsciiDoc documentation repos (modular docs with assemblies, includes, conditionals) |
| `jtbd-tools:jtbd-analyze-topicmap` | Extract JTBD records from OpenShift docs repos that use `_topic_map.yml` for book structure |
| `jtbd-tools:jtbd-toc` | Generate a JTBD-oriented Table of Contents from JTBD records |
| `jtbd-tools:jtbd-compare` | Compare current feature-based structure with proposed JTBD structure |
| `jtbd-tools:jtbd-consolidate` | Generate stakeholder-facing consolidation reports with gap analysis and navigation improvements |
| `jtbd-tools:jtbd-harvest` | Harvest technical documentation from a URL for analysis |
| `jtbd-tools:jtbd-workflow-topicmap` | End-to-end workflow for topic map repos: analyze, TOC, compare, consolidate in one command. Supports batch processing. |
| `jtbd-tools:jtbd-workflow-adoc` | End-to-end workflow for AsciiDoc repos: analyze, TOC, compare, consolidate in one command. Supports batch processing. |

## Installation

### Option A: Plugin marketplace (recommended)

Install the plugin from the redhat-docs-agent-tools marketplace:

```
/plugin install jtbd-tools@redhat-docs-agent-tools
```

This single install provides all 9 skills.

### Option B: Symlink into your project

Clone the repo and symlink skills into your project's `.claude/skills/` directory:

```bash
git clone <repo-url> ~/redhat-docs-agent-tools

mkdir -p your-project/.claude/skills

# Symlink all skills
for skill in ~/redhat-docs-agent-tools/plugins/jtbd-tools/skills/jtbd-*; do
  name=$(basename "$skill")
  ln -s "$skill" "your-project/.claude/skills/$name"
done
```
...
cp -r redhat-docs-agent-tools/plugins/jtbd-tools/skills/jtbd-analyze your-project/.claude/skills/
cp -r redhat-docs-agent-tools/plugins/jtbd-tools/skills/jtbd-toc your-project/.claude/skills/
```
