...
### Step 9: Validate Against Source (Grounding Check)

After all records are extracted, verify each record is grounded in the source content. This step runs by default and can be skipped with `--skip-validation`.

#### 9a: Resolve Evidence Citations

For each record, parse the `evidence` field to extract:
- Source file name
- Section heading
- Line range

Re-read the cited lines from the reduced/combined file, expanding ±20 lines to capture surrounding context.

#### 9b: Section Existence Check (Mechanical)

Verify that each record's `section` field matches an actual heading in the reduced/combined file. Flag mismatches as `no_evidence`.

#### 9c: Run Grounding Check

Pass each record and its source excerpt to a focused validation prompt. For each record, evaluate three checks:

| Check | Question | Fail condition |
|-------|----------|----------------|
| **Grounded job** | Does the source content describe or imply this job/task/goal? | Job statement describes something not present in the excerpt |
| **Grounded persona** | Does the source content mention, imply, or address this persona/role? | Persona is invented (e.g., "SRE" when doc only says "administrator") |
| **Grounded outcomes** | Is each desired outcome derivable from the source content? | Outcome adds metrics, goals, or claims with no basis in the text |

**Strictness rule:** The source must contain or clearly imply the claim. Reasonable inference from explicit content is OK. Inventing new capabilities, roles, or metrics is NOT OK.

#### 9d: Classify Results

For each record, set:
- `validation_status`: `"pass"` (all checks pass), `"flag"` (one or more checks fail), or `"no_evidence"` (evidence can't be resolved)
- `validation_flags`: array of specific issues for flagged records

#### 9e: Processing Strategy for Validation

| Record count | Strategy |
|--------------|----------|
| ≤ 20 records | Single validation pass — batch all records with their source excerpts into one prompt |
| > 20 records | Chunked validation — group 5-10 records per validation subagent |

#### 9f: Validation Prompt Template

```
You are a grounding validator. For each JTBD record below, determine whether it is
supported by the source documentation excerpt.

For each record, output a JSON object with:
- record_index: (integer)
- job_grounded: true/false
- job_issue: (string, only if false — explain what's not in the source)
- persona_grounded: true/false
- persona_issue: (string, only if false)
- outcomes_grounded: true/false
- outcomes_issue: (string, only if false — list which outcomes lack support)

Be strict: the source must contain or clearly imply the claim.
Reasonable inference from explicit content is OK.
Inventing new capabilities, roles, or metrics is NOT OK.

## Records to validate

<records with their source excerpts>
```

#### 9g: Report Summary

Print a summary to the user after validation:

```
Validation: 18 pass, 3 flagged, 1 no_evidence
Flagged records: #4 (persona), #11 (outcome), #15 (job_statement)
```
