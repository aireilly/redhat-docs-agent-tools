...
```json
{
  ...
  "properties": {
    ...
    "validation_status": {
      "type": ["string", "null"],
      "enum": ["pass", "flag", "no_evidence", null],
      "description": "Result of grounding validation against source content"
    },
    "validation_flags": {
      "type": "array",
      "items": { "type": "string" },
      "description": "Specific grounding issues found during validation"
    }
  }
}
```
...
These fields are populated by the grounding validation step (Step 9 in methodology). Validation runs by default; use `--skip-validation` to bypass.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `validation_status` | enum/null | null | `"pass"`, `"flag"`, or `"no_evidence"`. Null if validation was skipped. |
| `validation_flags` | string[] | [] | Specific grounding issues (e.g., "persona 'SRE' not found in source") |
...
8. **validation_status** should be `"pass"` for final outputs (flagged records should be reviewed)
9. **validation_flags** should be empty for passing records
